...
void Game::update()
{
	...
	for (auto &p : players)
	{
		Collision::PlayerWithWallCollision(p->getComponent<ColliderComponent>(), p->getComponent<TransformComponent>());
	}
	Collision::PlayerWithWallCollision(ball->getComponent<ColliderComponent>(), ball->getComponent<TransformComponent>());
	manager.update();
}
